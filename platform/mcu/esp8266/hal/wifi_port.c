...
bool ICACHE_FLASH_ATTR start_wifi_ap(const char * ssid, const char * pass){
    ...
    printf("this is the sssid name :%s\r\n",ssid);
    sprintf(config.ssid, "adh_a1sj8WGN4yq_3103EE"/*ssid*/);
    if(pass){
...
static int  start_wifi_ap1(hal_wifi_module_t *m, const char *ssid, const char *passwd, int interval, int hide){
    WIFI_MODE mode = wifi_get_opmode();
    if((mode & SOFTAP_MODE) == 0){
        mode |= SOFTAP_MODE;
        if(!wifi_set_mode(mode)){
    ...
    if(passwd){
        sprintf(config.password, passwd);
    }
    config.max_connection = 4;
    config.beacon_interval = interval;
    config.ssid_hidden = hide;
    return wifi_softap_set_config(&config);
}

static int stop_wifi_ap1(hal_wifi_module_t *m){
    WIFI_MODE mode = wifi_get_opmode();
    mode &= ~SOFTAP_MODE;
    if(!wifi_set_mode(mode)){
        printf("Failed to disable AP mode!\n");
...
void config_wifi_mode(int mode)
{
    //wifi_set_mode(0x03);
    wifi_set_mode(mode);
}
...
hal_wifi_module_t aos_wifi_esp8266 = {
    ...
    .start_ap            = start_wifi_ap1,
    .stop_ap             = stop_wifi_ap1
};
