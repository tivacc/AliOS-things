...
static int ota_parse(void* pctx, const char *json)
{
    ...
    if(!ctx) {
        OTA_LOG_E("ota parse ctx boot_parma err");
        ret = OTA_PARAM_FAIL;
        goto parse_failed;
    }
    char *url = ctx->url;
    ...
    if(!url || !hash || !ota_param) {
        OTA_LOG_E("ota parse hase boot_parma err");
        ret = OTA_PARAM_FAIL;
        goto parse_failed;
    }
    ...
    if (!root) {
        OTA_LOG_E("ota parse root boot_parma err");
        ret = OTA_PARAM_FAIL;
        goto parse_failed;
    } else {
...
static void ota_download_thread(void *hand)
{
    ...
    if(ret < 0) {
        OTA_LOG_E("tesp point1.\n");
        ctx->upg_status = OTA_DOWNLOAD_FAIL;
        goto ERR;
    }
    ret = ota_malloc_hash_ctx(ctx->hash_type);
    if (ret < 0) {
        OTA_LOG_E("ota_download_thread boot_parma err");
        ret = OTA_PARAM_FAIL;
        ctx->upg_status = OTA_DOWNLOAD_FAIL;
        goto ERR;
    }
